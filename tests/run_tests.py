...
@test("MCP session registry isolates sessions")
def _():
    ...
    ga = registry.get("session-A")
    gb = registry.get("session-B")
    assert ga is not gb
    assert registry.get("session-A") is ga
    ga.check_tool("search", args={"q": "test"})
    ga.record_result(ok=True)
    assert ga.tool_calls_executed == 1
    assert gb.tool_calls_executed == 0

@test("MCP session registry max capacity evicts oldest")
def _():
    from aura_guard.adapters.mcp_adapter import _SessionRegistry
    registry = _SessionRegistry(
    ...
    registry.get("s3")  # evicts s1
    assert registry.active_session_count == 2
    g1_new = registry.get("s1")
    assert g1_new is not g1  # new instance
